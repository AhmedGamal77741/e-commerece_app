...
class FeedController extends ChangeNotifier {
  ...
  Future<void> markPostNotInterested({required String postId}) async {
    ...
    // Optionally remove from local list and notify
  ...
  Future<void> blockUser({required String userIdToBlock}) async {
    ...
    // Remove all posts from this user locally
    _posts.removeWhere((key, value) => value['userId'] == userIdToBlock);
    notifyListeners();
  }

  Future<void> unblockUser({required String userIdToUnblock}) => _repository.unblockUser(userIdToUnblock: userIdToUnblock);

  Future<void> reportUser({required String reportedUserId, required String postId}) => 
      _repository.reportUser(reportedUserId: reportedUserId, postId: postId);

  Future<void> uploadPost({required String text, required List<String> imgUrls, String? categoryId}) =>
      _repository.uploadPost(text: text, imgUrls: imgUrls, categoryId: categoryId);

  Future<void> updatePost({required String postId, required String text, required List<String> networkImgUrls, required List<XFile> newImages, String? categoryId}) =>
      _repository.updatePost(postId: postId, text: text, networkImgUrls: networkImgUrls, newImages: newImages, categoryId: categoryId);
  ...
  Future<String> uploadSingleImageToFirebase(XFile image, int index, {Function(double)? onProgress}) =>
      _repository.uploadSingleImageToFirebase(image, index, onProgress: onProgress);
