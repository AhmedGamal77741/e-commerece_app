...
Future<void> deleteFriend(
  ...
  final displayName = await ref
      .read(friendsControllerProvider.notifier)
      .getDisplayName(friend.userId, friend.name);
  ...
  try {
    ...
    await ref
        ...
        .softDeleteChatForCurrentUser(chatRoomId);
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
...
void showBioEditDialog(
  BuildContext context,
  WidgetRef ref,
  MyUser? currentUser,
  Function(MyUser) onUpdate,
) {
...
void showChangeNameDialog(
  ...
  showDialog(
    ...
    builder:
        (dialogContext) => Dialog(
          backgroundColor: Colors.white,
          insetPadding: EdgeInsets.symmetric(horizontal: 24.w, vertical: 80.h),
          shape: RoundedRectangleBorder(
                        ...
                        onPressed: () async {
                          ...
                          try {
                            if (newAlias.isEmpty) {
                              ...
                              await ref
                                  .read(friendsControllerProvider.notifier)
                                  .setAlias(friend.userId, newAlias);
                              if (context.mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
